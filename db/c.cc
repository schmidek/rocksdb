...
rocksdb_column_family_handle_t** rocksdb_create_column_families(
  ...
  SaveError(errptr,
      db->rep->CreateColumnFamilies(ColumnFamilyOptions(column_family_options->rep),
        names, &handles));

  rocksdb_column_family_handle_t** c_handles = static_cast<rocksdb_column_family_handle_t**>(malloc(sizeof(rocksdb_column_family_handle_t*) * num_column_families));
  for (int i = 0; i != num_column_families; ++i) {
...
void rocksdb_create_column_families_destroy(rocksdb_column_family_handle_t** list) {
  free(list);
}
